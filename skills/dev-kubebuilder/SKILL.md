...
- [makefile-patching](references/makefile-patching.md) — patching kubebuilder Makefile to go run style, per-environment variants
- [kustomize-config](references/kustomize-config.md) — reviewing generated kustomize configuration
...
Read [kustomize-config](references/kustomize-config.md). Verify namespace, resource limits, and webhook patches in the generated kustomize configuration.
...
Read [makefile-patching](references/makefile-patching.md). Patch kubebuilder's generated Makefile to follow `dev-go-project-new` conventions: `go run <module>@<version>` for all tools, pinned `_VERSION` variables, no `bin/` directory. The reference covers envtest, k3senvtest, and kind variants.
